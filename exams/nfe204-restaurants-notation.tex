...
à déterminer la note moyenne donnée par chaque internaute, en fonction de
certains critères comme la catégorie du restaurant.
...
\begin{enumerate}
  \item Les lignes de \texttt{Restaurant} et les lignes de \texttt{Notation} 
   ...
   \item Les notations d'un restaurant sont sur un seul serveur.
   \item Les notations d'un même internaute sont sur un seul serveur et triées sur l'identifiant du restaurant.  
\end{enumerate}  

  \item On veut trouver efficacement la moyenne des notes d'un restaurant avec 
  la requêtes CQL suivante:
  \begin{minted}{sql}
    select avg(note) from Restaurant where nom='Il pastificcio';
  \end{minted}
  Le schéma proposé est-il adapté\,? Si non, indiquez les modifications
  nécessaires.
  \item Proposez un second schéma qui permet de trouver efficacement 
     la moyenne des notes données par un internaute.
\end{enumerate}
...
\begin{enumerate}
  \item Indiquez
    ...
    permet de produire les lignes à insérer dans la table Cassandra \texttt{Restaurant}
    donné dans la section précédente. 

\item Voyez-vous comment, avec un seul traitement MapReduce, insérer à la fois 
   dans les tables \texttt{Restaurant} et dans \texttt{Notation} à partir
   de notre collection de documents JSON\,? Proposez une solution.
\end{enumerate}
...
\section{Recherche d'information (5 pts)}
...
\begin{enumerate}
  \item Expliquez comment un document de notre collection A de documents 
     ...
     faire avec chacune des deux solutions envisagées, ElasticSearch et Cassandra,

...
\section*{Première partie : modélisation semi-structurée (4 pts)}
...
et la liste des articles achetés avec, pour chacun, la quantité et le prix unitaire. 
...
\begin{enumerate}
  \item Dans une base relationnelle, combien de tables faudrait-il pour représenter 
    ...
    \item Si on choisit une base NoSQL, comment représenter un ticket de caisse en JSON.
        Donnez un exemple simple. Combien d'insertions faut-il faire pour créer un ticket\,?
   \item Je veux compter le nombre de transactions par caisse et par jour.  Quelle est la requête SQL si les données
    ...
    \item Même question avec MapReduce\,: comment obtenir le nombre de transactions par caisse et par jour
      par un calcul distribué. Donnez les fonctions en pseudo-code ou, à défaut, une explication très précise.
 \end{enumerate}
...
\section*{Deuxième partie : calcul sur flux (3 pts)}

Je veux obtenir, en continu, le montant total perçu par chaque caisse depuis l'ouverture du magasin
le matin. Les tickets arrivent dans un flux réinitialisé chaque jour, et vous disposez
d'un moteur de traitement distribué  de flux  ayant les mêmes fonctionnalités que celles
 vues en cours.

\begin{enumerate}
  \item Décrivez le \textit{workflow} qui implante ce calcul. Préciser les opérateurs, leur rôle,
      la nature des flux en entrée et en sortie.
    \item Quel est le degré maximal de parallélisation de ce traitement (expliquez...)?
\end{enumerate}
...
\begin{enumerate}
       ...
       \item Que faut-il faire pour étendre le nombre de \textit{shards}\,?
       \item Que faut-il faire pour  étendre le nombre de réplicas\,?
...
\section*{Quatrième partie : structures distribuées (2 pts)}

On met en place un système MongoDB qui applique un partitionnement par intervalle. La clé est 
l'identifiant des documents, engendrés par incrémentation d'un compteur.


\begin{enumerate}

  \item On s'aperçoit que l'opération de \textit{split} est toujours effectuée par le même serveur.
      Comment expliquez-vous cette situation?

  \item Si je mets en place un partitionnement par hachage, est-ce que je résous ce problème? Expliquez.

 \end{enumerate}

\section*{Cinquième partie : questions de cours (3 pts)}

...
\date{Examen session hybride\,: 21 avril 2026}
...
\begin{solution}
	...
	\begin{enumerate}
	   \item Chaque serveur est responsable d'un secteur sur l'anneau. L'identifiant
	       d'un document est haché et la valeur de hachafe détermine donc le serveur d'accueil.
		   
	  \item Le structure d'anneau évite d'avoir à gérer le cas des extrêmités d'une structure linéaire. 
	  \item Comme expliqué en cours, on ne peut pas modifier la fonction de hachage. Autant donc
	      choisir dès le départ une fonction maximale et immuable. La règle
		   d'affectation sur l'anneau fournit l'adaptation au nombre de serveurs.
	   \item Les n{\oe}uds virtuels améliorent la distribution en tendant vers
	     une nombre moyen de document sur chaque serveur
	\end{enumerate}

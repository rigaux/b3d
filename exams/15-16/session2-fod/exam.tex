...
\date{Examen seconde session FOD: vendredi 9 septembre 2016}
...
\begin{enumerate}
  \item Donnez une première représentation de l'échantillon ci-dessus dans laquelle chaque document
       représente une entité "client". 
      Nous l'appellerons représentation A.
  \item Proposez une autre représentation, toujours en JSON, centrée non plus sur les clients
       mais sur les entités "conducteur".  Nous l'appellerons représentation B.
  \item On veut calculer par un traitement MapReduce le
       tarif moyen des trajets par conducteur.
      Choisissez la représentation (A ou B) la plus adapté du point de vue des performances, 
      et donnez les fonctions de Map et de Reduce dans la syntaxe de votre choix.
      
  \item Justifiez le choix de la représentation, A ou B, pour le calcul précédent. 
       Indiquez en particulier où se situe le gain de performance attendue pour la représentation
       choisie: au niveau du Map? du Reduce? Autre? 
...
\section*{Troisième partie : Pig et MapReduce (3 pts)}

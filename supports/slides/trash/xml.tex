...
\section{Les bases du formalisme XML}
...
Version : 1.1 : diff\`ere avec des fonctionalit\'es suppl\'ementaires concernant des caract\`eres "sp\'eciaux" (genre fin de ligne "ascii"). Conseill\'e que si besoin de ces fonctionnalit\'es suppl\'ementaires; en gros, quasi tout le monde travaille en 1.0
2.0 à l'\'etude\\
Standalone, relevant que si une DTD est d\'eclar\'ee en externe du document (sinon, ignoré). Attribut g\'en\'eralement omis car optinnel.
Default = no. it is a signal to the XML processor that the DTD is not just for validation: there could be things to look up in the DTD like:Default values for attributes Entity declarations.
}
...
\item d’instructions de traitement (par ex. appel à XSLT) \verb7<?instruction attribut1 = "valeur" ... attribut n ="valeur" ? >7 \prof{Par exemples : 1) Instruction de traitement, par exemple en HTML, il y avait ça pour inclure du code PHP. 2) feuille de style XSL associé à un fichier XHTML.}
\end{itemize}
\end{enumerate}
...
\end{verbatim}
Remarque : au sein d’\texttt{elt\_feuille} et d'\texttt{elt\_noeud} toutes les occurrences de \texttt{nom\_elt} prennent la m\^eme valeur. 
...
\begin{frame}[fragile]
\frametitle{The syntax for serialized document, in brief}
...
Last example shows the \emph{prologue}, useful for XML parsers (it
gives in particular the document encoding).

\end{frame}


\begin{frame}[fragile]
\frametitle{From serialized to tree form: text and elements}

\begin{tabular}{m{.4\hsize}m{.5\hsize}}
    
The basic components of an XML document are {\em element} and 
{\em text}. 

Here is an \emph{element}, whose content is a \emph{text}.
...
\end{tinyverbatim}
 ...
 & 
  The tree form of the document, modeled in DOM: 
  each node has a \domtype{type}, either \domtype{Document}
   or \domtype{Text}.
...
\begin{frame}[fragile]
\frametitle{From serialized to tree form: nesting elements}

The content of an element is
\begin{enumerate}
  \item  the part between the opening and ending tags (in serialized form), 
  \item  the subtree rooted at the corresponding \nodetype{Element} node
     (in DOM).
\end{enumerate}

The content may range from atomic text, to any recursive
combination of text and elements (and gadgets, e.g., comments).
...
%%%%%%%%%%%%% 
\begin{frame}[fragile]
\frametitle{From serialized to tree form: attributes}

\emph{Attributes} are pairs of name/value attached to an element.
\begin{enumerate}
  \item as part of the opening tag in the serialized form, 
  \item as special child nodes of the \nodetype{Element} node
     (in DOM).
\end{enumerate}
...
\end{tinyverbatim}
  ...
  Unlike elements, attributes are \emph{not} ordered,
  and there cannot be two attributes with the same
  name in an element.
...
%%%%%%%%%%%%% 
\begin{frame}[fragile]
\frametitle{From serialized to tree form: the document root}

The first line of the serialized form must \emph{always} be the
\emph{prologue} if there is one:

\xmlinline{<?xml version="1.0" encoding="utf-8" ?>}

and the document content must \emph{always} be enclosed in
a single opening/ending tag, called the \emph{element root}.

\vspace*{-1em}
\begin{columns}

\column{.5\linewidth}
A document with its prologue, and element
root.
...
objects after the prologue (processing instructions).

\column{.5\linewidth}

 In the DOM representation, the prologue appears as
  a \nodetype{Document} node, called the \emph{root node}.
...
\begin{frame}
\frametitle{Summary: syntax and vocabulary}


\textbf{Serialized form}
\begin{itemize}
  \item A document begins with a prologue,
  \item It consists of a single upper-level tag, 
  \item Each \emph{opening tag} \texttt{<name>}
      has a corresponding \emph{closing tag} \texttt{</name>};
      everything between is either text or properly enclosed
         tag content.
\end{itemize}

\textbf{Tree form}
\begin{itemize}
  \item A document is a tree with a \emph{root node} (\nodetype{Document}
          node in DOM),
  \item The root node has one and only one element child
              (\nodetype{Element} node in DOM), called the 
             \emph{element root})
  \item Each element node is the root of a \emph{subtree}
            which represents its structured \emph{content}
\end{itemize}

\end{frame}

\begin{frame}
\frametitle{Summary: syntax and semantics}

XML provides a syntax

\vfill
The core of the syntax is the \alert{element name}
...
\subsection{XML Syntax: Complements}

%%%%%%%%%%%%%%%%%%%%%%%


\begin{frame}[fragile]
\frametitle{Entities and references}

Entities are used for the physical organization of a document.

\vfill

An entity is \alert{declared} (in the document type), then
\alert{referenced}.
...
%------------------------------------------------------------
\begin{frame}
\frametitle{Predefined entities}

Several symbols cannot be directly used in an XML
document, since they would be misinterpreted by the parser.
...
\begin{tabular}{l|l|l}
{\bf Declaration} & {\bf Reference}&{\bf Symbol.}\\ 
\hline 
...
%------------------------------------------------------------
\begin{frame}
\frametitle{Comments and instructions}

\alert{Comments} can be put at any place in the serialized form.

\vfill
   \begin{center}
      \xmlinline{<!{-}- This is a comment  {-}->}
    \end{center}
\vfill

   They appear as \nodetype{Comment} nodes in the DOM tree (they are
   typically ignored by applications).

\vfill

\alert{Processing instructions}: specific commands, 
            useful for some applications, simply
          ignored by others.
\vfill
The following instruction requires the transformation of the document
...
%------------------------------------------------------------
\begin{frame}[fragile]
\frametitle{Literal sections}
Usually, the content of an element is \emph{parsed}
to analyse its structure.

\vfill 
\alert{Problem}: what if we do \emph{not} want the content to be parsed?
...
\alert{Solution}: use entities for all special symbols; or 
prevent parsing with a \emph{literal section}.
...
Quelques remarques : 
\begin{itemize}
\item Les sous-\'el\'ements sont ordonnés. \prof{on pourra demander le 1er sous-elt quand on interroge. Mais pas oblig\'e d'en tenir compte.} 
...
\item Les caractères spéciaux non autorisés peuvent \^etre remplac\'es par une r\'ef\'erence:\\ \prof{ou r\'ef. num\'erique, ou un verbatim \'ecrit en XML par \verb?<![CADATA[ bla bla bla]]>?}
...
\item pr\'esence d'\textbf{un seul \'el\'ement racine},
...
int\'egrant g\'en\'eralement un parser XML (pour pouvoir \^etre pars\'e, un document doit \^etre bien form\'e).
}
...
\smallskip
 ...
 Il obtient son doctorat en philosophie en 1930. Il y prouve la compl\'etude de la logique classique du premier ordre. Il est particuli\`erement c\'el\`ebre pour son ``th\'eor\`eme d'incompl\'etude'' publi\'e en 1931.\\
...
\lstinputlisting[caption={Extrait (premi\`eres lignes) d'un fichier xml},label={pageIssueDeListe11},breaklines=true,inputencoding=utf8/latin1,lastline=18,numbers=left]{codes/pageIssueDeListe.xhtml}
...
\begin{frame}[containsverbatim, fragile]{Quelques exemples d'utilisation des espaces de noms : les dialectes}
\figSized{images/pageIssueDeListe.jpg}{Fichier contenant le Listing \ref{pageIssueDeListe11}, ouvert sous Firefox (v 6.0.1)}{0.7}
\end{frame}

\begin{frame}[containsverbatim, fragile]{Quelques exemples d'utilisation des espaces de noms : les dialectes}
\exo{Que se passe-t-il lors de l'ouverture sous Firefox si la ligne 2 du listing \ref{pageIssueDeListe11} est remplacée par la ligne suivante ?\\ ~\\
{\small \texttt{2}}~~~ \verb?<html>? }

\prof{\figSized{images/pageIssueDeListe_ssns.jpg}{Fichier contenant le Listing \ref{pageIssueDeListe11} sans namespace, ouverte sous Firefox (v 6.0.1)}{0.5}
}
\end{frame}
...
\verb7<!ELEMENT auteur (nom, prenom)>7 ~~~~~~$\leftarrow$ \textit{s\'equence}\\
\verb7<!ELEMENT cours (magistral | projet)>7 ~~~~~~$\leftarrow$ \textit{choix}\\
...
\verb7<!ELEMENT nom children>7 où \\
...
Il doit exister au plus une déclaration \verb7<!ELEMENT ..>7 par élément. \\
Tous les éléments doivent \^etre d\'efinis.
...
\item $\mathtt{occurence\_i ~(1 \leq i \leq n)}$ est l'occurence associée à l'attribut $\mathtt{nom\_att\_i}$ et prend sa valeur dans :
  ...
  \item[\texttt{\#FIXED valeur}] = valeur fixée (pas de valeur par d\'efaut)
  \item[(vide)] : par défaut, \texttt{\#REQUIRED}
  \end{description}
Contrainte pour la DTD : si $\mathtt{type\_i ~(1 \leq i \leq n)} = ID$ alors $\mathtt{occurrence\_i} \in \{\mathtt{\#REQUIRED}, \mathtt{\#IMPLIED}\}$.
...
Des exemples classiques de clef sont le numéro de sécurité sociale, le numéros ISBN d'un libre, le numéro d'une carte bleue, ou encore la référence des produits dans un catalogue de vente par correspondance.\\
...
\begin{exoblock}
Auquel (Auxquels) des attributs ci-dessous vous semble-t-il pouvoir associer le type ID ? \prof{ISBN}
\end{exoblock}
\end{frame}
...
Spécifie un ID unique au sein du document XML entier, dont la portée est tout le document $\rightarrow$ Il ne doit pas exister deux valeurs identiques d'ID (tous élements confondus) au sein d'un même document (nous ne sommes pas habitués à cela dans le monde des BD).\\
\end{alertblock}
\end{frame}
...
\medskip
\href{open : ./codes/films_le_pb.xml}{Le problème : fichier \textit{films\_le\_pb.xml}} (fichier disponible sur Plei@d)

\href{open : ./codes/films_correction.xml}{Une correction : fichier \textit{films\_correction.xml}} (fichier disponible sur Plei@d)
...
\end{lstlisting}
La ligne 2 indique au processeur \prof{ou parser}1) qu'une DTD est associée et 2) où la trouver.
...
\begin{frame}{Deux types de vérifcation de schéma (typage)}
 Deux types de vérification :

\begin{description}
\item[Vérification dynamique] Un validateur vérifie le type des documents en entrée et éventuellement en sortie de l'application. 
	Au moment de l'exécution.
\item[Vérification statique] On vérifie que le programme (l'application) sachant qu'elle prend un certain type (ou un certain schéma) en entrée, renvoie bien le type attendu en sortie.
	Analyse avant exécution, à la "compilation" (partiel). 
\end{description}

\prof{Nous en verrons un exemple de v\'erification statique avec Oralce XML DB mais apr\`es avoir vu XQuery}
...
\begin{frame}[containsverbatim]{Et la s'émantique dans tout ça ?}
\begin{lstlisting}
<!DOCTYPE fiches [
...
\begin{frame}[allowframebreaks]{Document ou données ?}
Distinction courante :
\begin{itemize}
\item fichier XML orienté document : article, news, ouvrage, etc
\item fichier XML orientés données : typage fort, stockage d'informations ayant une structure commune, modélisable par un schéma
\end{itemize}
...
\lstinputlisting[caption={Orient\'e donn\'ees},label=pageIssueDeListe,breaklines=true,inputencoding=utf8/latin1,lastline=18,numbers=left]{codes/listeUML.xml}

\framebreak

\figSized{images/docoriented.jpg}{Orient\'e document}{0.8}

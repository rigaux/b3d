...
Pour l'installation, plusieurs possibilités\,:
...
\begin{redbullet}
  ...
  \item Installer directement le code ElasticSearch (compliqué)
  \item Installer localement avec Docker\,: très utile et instructif
\end{redbullet}

\vfill

Recommandation\,: commencer par la première solution (plus sûr) puis
essayez une des autres si vous êtes motivés.

\end{frame}


\begin{frame}[fragile]{Création d'un serveur dans le cloud ElasticSearch}

Rendez-vous sur \url{https://cloud.elastic.co/} et créez un compte.

\vfill

Créez un index \texttt{films}, notez l'URL et la clé d'accès.

\vfill

\figSlideWithSize{cloud-elastic}{10cm}
...
Avec Docker, les machines virtuelles sont des \alert{conteneurs}.

\vfill

\figSlideWithSize{Serveurs}{11cm}
...
9200, \alert{dans le conteneur}, qui a sa propre adresse Internet.
...
\alert{Réponse}: \emph{il faut associer un port de la machine hôte avec le port 
...
Par exemple, 
\begin{redbullet}
 \item j'associe  le port 9200 de la machine hôte avec le port 9200 du conteneur \texttt{elastic1}
 \item j'associe  le port 9201 de la machine hôte avec le port 9200 du conteneur \texttt{elastic2}
\end{redbullet}
...
\begin{redbullet}
	...
	\item Récupérez l'application \textit{Docker desktop} poiur votre système et lancez-la
\end{redbullet}

\vfill

\centerline{\figSlideWithSize{docker-dashboard.jpg}{8cm}}
...
\begin{frame}{Le serveur est prêt\,? Il nous faut un client}

Nous allons utiliser ElasticVue (\url{https://elasticvue.com/})

\vfill

Installez l'application et connectez-vous au serveur avec l'URL et l'authentification.

\vfill

\centerline{\figSlideWithSize{elasticvue.png}{10cm}}

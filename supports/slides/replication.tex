...
\begin{frame}
\frametitle{Le principe du quorum}

On peut gérer la cohérence plus finement grâce au paramétrage suivant\,:

\vfill

\begin{redbullet}
  \item $W$ le nombre *d'écritures* synchrones avant acquittement au client
   
  \item $R$ le nombre de *lectures*  synchrones avant acquittement au client par renvoi de la copie la plus récente

  \item $RF$ le facteur de réplication.
\end{redbullet}

$W$ et $R$ varient entre 1 et $RF$
\vfill

\begin{remblock}{Critère de cohérence forte}
...
\begin{frame}
\frametitle{Illustration du quorum}

Avec $W=2$, $R=3$ et $RF=4$.

\vfill

\figSlideWithSize{replication-quorum}{11cm}

\vfill
$\Rightarrow$ toute lecture accèdera à \alert{au moins}
une des versions cohérentes.
